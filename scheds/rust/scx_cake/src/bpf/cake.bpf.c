...
/* C1 PADDED: packed tier+WSC array for snapshot reader.
 * u16 per CPU: lower byte = tier, upper byte = WSC.
 * Padded to 64 bytes per entry to prevent false sharing during WSC updates.
 * WSC updates in select_cpu are frequent; without padding, they dirty cache
 * lines shared with other CPUs, stalling readers/writers. */
struct tier_wsc_entry {
    u16 data;
    u8 _pad[62];
} __attribute__((aligned(64)));
static struct tier_wsc_entry packed_tier_wsc[CAKE_MAX_CPUS] SEC(".bss");
                   ...
                   u64 wake_flags)
{
    ...
    if ((!restricted || bpf_cpumask_test_cpu(prev_cpu, p->cpus_ptr)) &&
        ...
        if (wsc < 255) {
            ...
            packed_tier_wsc[prev_idx].data = (packed_tier_wsc[prev_idx].data & 0x00FF) | ((u16)new_wsc << 8);
        }
     ...
     * Sim: -100% gate cascade jitter (P99-P50 → 0ns). */
    {
        ...
        if (wsc >= 8) {
            ...
            packed_tier_wsc[prev_idx].data &= 0x00FF;  /* C1: clear WSC upper byte */
...
void BPF_STRUCT_OPS(cake_running, struct task_struct *p)
{
    ...
    if ((packed_tier_wsc[cpu].data & 0xFF) != tier) {
        /* C1: write tier to lower byte, preserve WSC in upper byte */
        packed_tier_wsc[cpu].data = (packed_tier_wsc[cpu].data & 0xFF00) | (tier & 0xFF);
