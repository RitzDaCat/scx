# scx_cake — STATE archive

Settled entries moved out of `STATE.md`, word for word, newest first. Search it with
grep; do not read it whole. `STATE.md` holds what is current and states the offload
rule.

---

## Moved 2026-09-26: the §G97 entry (settled: §G97 removed, see STATE.md 2026-09-26)

**2026-09-23 — G97 ISR-WAKE PLACEMENT (LDAT-style input latency).** Cake steered
every wake away from IRQ-bad CPUs, including a short task woken by that CPU's own
interrupt. The CPU is awake and holds the event's lines, so moving the wake to another
idle CPU only paid a halt exit. `cake_isr_wake_cpu` places a non-stage wakee on the idle
waking CPU when its idle claim succeeds; stage tasks keep the IRQ steering.

| commit | what | receipt |
|---|---|---|
| `12803f341` | `cake_isr_wake_cpu` (noinline, called only for non-stage wakes, before seat retake); `isr_wake` census counter | select_cpu 10/30/682 → 10/27/701; helper 1/1/71; attach OK, verifier accepted |

Measured with OSLTT (hardware input-latency tester, Feather M0, firmware v3): the device
sends a HID report, a thread blocked on the device's evdev node wakes and echoes one byte
back; the device times USB report fetch → echo on its own clock. Busy Vulkan render
thread (~30k fps) running, 16-CPU 9800X3D, no cpuidle driver, 3 interleaved rounds × 150
shots per arm. Two runs:

| arm | run 1 p99 µs | run 1 >100 µs | run 2 p99 µs | run 2 >100 µs | run 2 mean µs |
|---|---|---|---|---|---|
| EEVDF | 60.5 | 3/450 | 67.0 | 3/450 | 41.2 |
| cake 1.2.1 (installed) | 136.7 | 12/450 | 157.0 | 34/450 | 49.3 |
| cake + G97 | 57.0 | 3/450 | 65.5 | 3/450 | 43.1 |

Run 1 used the inline form (same logic, `bdf0c14e4`); run 2 is `12803f341`. FRAME gate
owed: `cakebench game ab` against installed cake.

---

## Moved 2026-09-25: rounds 2026-09-19 and older, campaign, scoreboard, ledger, § registry

The block below is the former `STATE.md` from its first line, less the four
2026-09-22/23 entries that stayed in `STATE.md`.
...
2026-09-25: the 44 commits `12803f341` … `f397452b5` squashed into six checkpoint
commits, one per round (G97; readability refactor; open-item fixes; 09-24 instruction
round; release without the probe; 09-25 shaves), the first five on the tree of their
round's last commit, the sixth on `f397452b5`'s tree plus this note; pre-squash
history in `keep/pre-squash-2026-09-25`.
...
| `5d92f48c6` | **§G96 hardware arms, inert here by rodata gates**: capacity-first cold pick on hybrid parts (`cake_cap_word`, one cache domain only); cpuidle driver + per-state exit latency in rodata, `tp_btf/cpu_idle` attached only with a driver → `cake_deep_idle_word` (state exit > handoff floor) avoided like IRQ-hot cores (the pool kick pickers too since `97e8898a3`); multi-LLC: a stage thread off `cake_llc_pref_word` re-homes when a whole core is free there, workers follow the waker's die; V-cache mode re-read each second |
...
|---|---|---|---|---|
| **G97** | **ISR-wake placement: a non-stage task woken by an interrupt on an idle CPU claims that CPU (`12803f341`)** | **OSLTT device-timed input wake (USB fetch → evdev reader echo), interleaved 3×150/arm, render hog running** | ✅ **KEPT 2026-09-23; FRAME owed** | p99 cake 136.7/157.0 → **57.0/65.5 µs** (EEVDF 60.5/67.0); >100 µs 12→3 and 34→3 of 450 |
