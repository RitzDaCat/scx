...
fn handle_process_event(
    ...
    let evt = unsafe { (data.as_ptr() as *const ProcessEvent).read_unaligned() };
    ...
    match evt.type_ {
        GAME_EVENT_EXEC => {
            ...
            info!(
                "BPF LSM: Candidate process: {} (pid={}, flags={:#x})",
            ...
            if let Some(game) = validate_game_candidate(pid, flags) {
                ...
                if prev != game.tgid {
                    info!(
                        "BPF LSM: Game detected: '{}' (pid={}, wine={}, steam={})",
...
fn validate_game_candidate(pid: u32, bpf_flags: u32) -> Option<GameInfo> {
    ...
    if has_mangohud_shm(pid) {
        score += 1000;
        info!(
            "BPF LSM: MangohHUD detected for pid {} ({}), strong game signal",
            pid, game_name
        );
    }
...
fn has_game_keywords_bytes(comm: &[u8]) -> bool {
    ...
    let game_patterns: &[&[u8]] = &[b"wine", b"proton", b".exe", b"game", b"warframe"];
    ...
    if comm_lower.windows(5).any(|w| w == b"steam") || comm_lower.windows(6).any(|w| w == b"reaper")
    {
        return true;
    }
...
fn detect_initial_game() -> Option<GameInfo> {
    ...
    for entry in proc_entries.flatten() {
        ...
        if let Some(game) = validate_game_candidate(pid, 0) {
            ...
            info!(
                "BPF LSM: Candidate '{}' scored {} (wine={}, steam={}, threads={:?})",
                game.name,
                score,
                game.is_wine,
                game.is_steam,
    ...
    info!(
        "BPF LSM: Initial scan complete in {:.2}ms (checked {} processes, validated {} candidates)",
    ...
    if let Some(ref game) = best_game {
        info!(
            "BPF LSM: Found running game: '{}' (pid={}, score={})",
