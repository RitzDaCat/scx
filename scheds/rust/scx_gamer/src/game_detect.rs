...
#[inline]
fn fallback_scan_due(
    last_run: &mut std::time::Instant,
    ...
    let mut inotify = match Inotify::init() {
        Ok(inotify) => {
            ...
            match fcntl::fcntl(fd, fcntl::FcntlArg::F_GETFL) {
                Ok(current_flags) => {
                    ...
                    match fcntl::fcntl(fd, fcntl::FcntlArg::F_SETFL(new_flags)) {
                        Ok(_) => {
                            match inotify
                                .watches()
                                .add("/proc", WatchMask::CREATE | WatchMask::ONLYDIR)
                            {
                                Ok(_) => {
                ...
                Err(e) => {
                    warn!(
                        "game detector: failed to get fd flags: {}, falling back to polling",
                        e
                    );
                    None
                }
        ...
        Err(e) => {
            warn!(
                "game detector: failed to init inotify: {}, falling back to polling",
                e
            );
            None
        }
...
fn handle_detection_result(
    ...
    match detection_result {
        Ok(Some(game)) => {
            ...
            if prev != game.tgid {
                info!(
                    "game detector: found game '{}' (tgid={}, wine={}, steam={})",
...
fn get_process_stats(pid: u32) -> Option<ProcessStats> {
    ...
    for line in status.lines() {
        if line.starts_with("Threads:") {
            threads = line.split_whitespace().nth(1)?.parse::<usize>().ok()?;
        } else if line.starts_with("VmRSS:") {
            vmrss_kb = line.split_whitespace().nth(1)?.parse::<u64>().ok()?;
        }
    }
...
fn calculate_score(game: &GameInfo) -> i32 {
    ...
    if has_mangohud_shm(game.tgid) {
        ...
        info!(
            "game detector: MangohHUD detected for PID {} ({}), strong game signal",
            game.tgid, game.name
        );
    }
        ...
        || (cmdline_lower.contains(".exe")
            && (
                cmdline_lower.contains("c:\\")
                    ...
                    || cmdline.contains(":\\")
                // Any Windows drive letter pattern
            ));
